 ...
 */
function _pingApi(aPIKey, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'System/API_Ping/{API_Key}/';
...
class GetSystemApi {
   ...
   */
  pingApiWithHttpOperationResponse(aPIKey, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._pingApi(aPIKey, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  pingApi(aPIKey, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._pingApi(aPIKey, options, optionalCallback);
    }
  }

}

module.exports = GetSystemApi;
